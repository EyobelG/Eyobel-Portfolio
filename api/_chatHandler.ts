...
// Lazy initialize the Gemini AI client so a missing key doesn't crash the process/module on load.
...
export async function handleChatRequest(messages: unknown): Promise<ChatResult> {
  if (!messages || !Array.isArray(messages)) {
    return { status: 400, body: { error: "Invalid request. 'messages' array is required in the body." } };
  }
